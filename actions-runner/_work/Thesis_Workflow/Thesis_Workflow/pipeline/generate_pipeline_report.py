...
SYSTEM_PROMPT = (
    ...
    "FORMATTING RULES:\n"
    "- Do NOT generate a top-level # heading — one is added automatically\n"
    "- Use ## for section headings (never # or ### or deeper)\n"
...
def gather_data_db_health(db_path):
    ...
    try:
        ...
        for row in cursor.fetchall():
            lines.append(f"  Study: {row[0]} ({row[1]}, {row[2]} HEEDS designs, {row[3]})")
        # Count total cases including baseline
        cursor.execute("SELECT COUNT(*) FROM cases")
        total_cases = cursor.fetchone()[0]
        cursor.execute("SELECT COUNT(*) FROM cases WHERE is_baseline = 1")
        baseline_count = cursor.fetchone()[0]
        lines.append(
            f"\nNote: Total cases in DB = {total_cases} "
            f"({baseline_count} baseline + {total_cases - baseline_count} HEEDS designs). "
            "Baseline (case 0) is imported separately and is NOT counted in the study's "
            "num_cases field. This is expected — not a mismatch."
        )
    except Exception:
        pass
    ...
    try:
        # Query stiffness parameters — check multiple naming conventions
        cursor.execute(
            "SELECT c.case_name, p.element_id, p.parameter_name, p.value "
            "FROM parameters p JOIN cases c ON p.case_id = c.case_id "
            "WHERE p.parameter_name LIKE '%K4%' "
            "   OR p.parameter_name LIKE '%stiffness%' "
            "   OR p.parameter_name LIKE '%PBUSH%' "
            "   OR p.parameter_name LIKE '%k4%' "
            "ORDER BY c.case_number"
        )
        rows = cursor.fetchall()
        if rows:
            lines.append("\nStiffness parameter values per case:")
            for row in rows:
                lines.append(f"  {row[0]}: element {row[1]}, {row[2]} = {row[3]}")
        else:
            # Show what parameter names actually exist
            cursor.execute(
                "SELECT DISTINCT parameter_name FROM parameters LIMIT 20"
            )
            param_names = [r[0] for r in cursor.fetchall()]
            lines.append(f"\nNo K4/stiffness parameters found. "
    ...
    try:
        cursor.execute(
            ...
            "   OR pk.response_type LIKE '%CBUSH%' "
            "   OR pk.response_type LIKE '%force%' "
            "   OR pk.response_type LIKE '%FORCE%' "
...
def gather_data_feature_matrix(data_file):
    ...
    # Check for label arrays: "y", "y_bolt", "y_severity", "y_binary", etc.
    label_keys = [k for k in data.keys() if k == "y" or k.startswith("y_")]
    if label_keys:
        for ykey in sorted(label_keys):
            y = data[ykey]
            lines.append(f"\n{ykey} shape: {y.shape}")
            unique, counts = np.unique(y, return_counts=True)
            lines.append(f"Label distribution ({ykey}):")
            for label, count in zip(unique, counts):
                lines.append(f"  Class {label}: {count} samples")
    else:
        lines.append("\nNo label arrays found in file.")
...
def write_report(output_dir, report_type, content):
    ...
    # Post-process: strip any top-level # heading the LLM generated
    # (write_report adds its own # title below)
    lines = content.split('\n')
    fixed_lines = []
    for line in lines:
        # Remove duplicate top-level headings (# Title but not ## Section)
        if line.startswith('# ') and not line.startswith('## '):
            continue
        fixed_lines.append(line)
    # Strip leading blank lines left by removed heading
