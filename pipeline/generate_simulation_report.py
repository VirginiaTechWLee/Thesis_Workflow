...
def generate_report(f06_content, f06_filename, model_input_content="", pch_content=""):
    ...
    user_prompt = (
        ...
        "2. **Natural Frequencies & Modal Effective Mass** — show only modes with "
        "frequency <= 2000 Hz. Modes above 2000 Hz are not structurally meaningful "
        "for this analysis and should be omitted. After the table, add a note: "
        "'Note: Only modes up to 2000 Hz are shown. N higher-frequency modes (up to X Hz) "
        "were extracted but omitted.' Include the MODAL EFFECTIVE MASS FRACTION data "
        "from the F06. Use this markdown table format:\n"
        ...
        "4. **Solution Sequence Status** — explicitly state which solution sequences ran "
        "(SOL 103 modal analysis AND/OR SOL 111 frequency response / random vibration). "
        "State whether each completed successfully.\n"
        "5. **Mode Shape Interpretation** — for each mode shown in the frequency table, "
        "describe the physical bending behavior: which direction it bends (X/Y/Z), "
        "whether it is a 1st, 2nd, or 3rd bending mode based on the MEMF participation "
        "factors and displacement patterns. Relate T1/T2/T3 dominance to physical motion. "
        "Keep it concise — 1-2 sentences per mode.\n"
        "6. **Random Response Summary** — if SOL 111 data is present, include:\n"
        "   - A **Peak Response Table** in markdown format:\n"
        "     | Node | Component | Peak Displacement (PSD) | Peak Acceleration (PSD) | Dominant Freq (Hz) |\n"
        "     Show the top 5-10 nodes with highest response. Include units if available.\n"
        "   - A brief interpretation of where the structure responds most and why.\n"
        "   - **Amplification Ratio** — compare tip (free-end) acceleration to the base "
        "input node acceleration. Report the ratio and note whether it indicates "
        "significant dynamic amplification.\n"
        "   - **Mode Contribution** — for each peak response row, identify which natural "
        "frequency (from the SOL 103 modal table) the dominant response frequency "
        "correlates to, and name the corresponding mode shape.\n"
        "   - **CBUSH Sensitivity** — if CBUSH stiffness data is provided, note how the "
        "bush stiffness values relate to the response pattern (e.g., softer bushings "
        "shifting peak frequencies, amplifying or attenuating tip response).\n"
        "   - **Engineering Takeaway** — provide a 2-3 sentence 'so what' conclusion: "
